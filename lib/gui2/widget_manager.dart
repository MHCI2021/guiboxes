...
class WidgetManager {
  ...
  Widget toRow(var items, {var additional}) {
    return Row(
      children: toWidgets(items, additonalWidgets: additional),
    );
  }
  ...
  Widget toRaisedButton(var name, {var extra}) =>
      toButton(name, extra: extra, type: "raisedButton");
  Widget toFlatButton(var name, {var extra}) =>
      toButton(name, extra: extra, type: "flatButton");
  ...
  Widget toButton(var name, {var extra, var type = "raisedButton"}) {
    ...
    var text = ifIs(info, "text") ??
        (curVal ? (ifIs(info, "trueText") ?? "") : ifIs(info, "falseText"));
    Widget btn;
    if (type == "flatButton") {
      btn = FlatButton(
          color: color,
  ...
  List<Widget> toUpDown(var name, {var extra}) {
    ...
    return [
      IconButton(
        onPressed: () {
          if (val >= min) updateVal(name, val - incr);
        },
      ...
      IconButton(
        onPressed: () {
          if (val <= max) updateVal(name, val + incr);
        },
  ...
  List<Widget> toWidgets(var items, {Map additonalWidgets}) {
    ...
    if (items is List)
      items.forEach((item) {
        ...
        if (map.containsKey(item)) out = tryAdd(out, toWidget(item));
      });
  ...
  dynamic toWidget(var name, {var extra}) {
    if (map[name].containsKey("type")) {
      ...
      if (type == "updownarrows")
        return toUpDown(name, extra: extra); // rename, info
      if (type == "flatButton")
        return toFlatButton(name, extra: extra); //, info
